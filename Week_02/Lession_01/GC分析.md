...
### 总结
从数据上看不同 GC 不同内存第一次的 QPS 都差不多 而第二次普遍少于第一次, 当时猜测是第一次没有进行 Full GC, 因为接口只是返回了一个 Hello world, 
后来开启 GC 日志发现, 第一次压测的确没有进行 Full GC, 只有 Young GC;

请求数最高值出现在 并行GC 内存1g 的配置上, 验证了 并行GC 吞吐量优先;
